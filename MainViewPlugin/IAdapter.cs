...
namespace Mcv.MainViewPlugin
{
    ...
    {
        public SiteAddedEventArgs(SiteId siteId, string siteDisplayName)
        {
            SiteId = siteId;
    ...
    class MessageReceivedEventArgs : EventArgs
    {
        ...
        public MessageReceivedEventArgs(NotifyMessageReceived messageReceived)
        {
    ...
    class IAdapter : IConnectionNameHost
    {
        ...
        internal void OnMessageReceived(NotifyMessageReceived messageReceived)
        {
            MessageReceived?.Invoke(this, new MessageReceivedEventArgs(messageReceived));
        }
        ...
        internal void OnBrowserAdded(BrowserProfileId browserProfileId, string browserDisplayName, string profileDisplayName)
        {
            BrowserAdded?.Invoke(this, new BrowserAddedEventArgs(browserProfileId, browserDisplayName, profileDisplayName));
        }

        //public bool IsShowConnectionName { get { return _options.IsShowConnectionName; } set { _options.IsShowConnectionName = value; } }

        internal void OnSiteAdded(SiteId siteId, string siteDisplayName)
        {
        ...
        {
            ConnectionAdded?.Invoke(this, new ConnectionAddedEventArgs(connSt));
        }
        ...
        {
            _host.SetMessage(new PluginV2.Messages.ToCore.RequestCloseApp());
        }
        ...
        internal void RemoveConnections(List<ConnectionId> selectedConnections)
        {
            ...
            {
                _host.SetMessage(new PluginV2.Messages.RequestRemoveConnection(connId));
            }
        }
        internal void RequestAddConnection()
        {
            _host.SetMessage(new PluginV2.Messages.RequestAddConnection());
        }

        internal void RequestChangeConnectionStatus(ConnectionStatusDiff connectionStatusDiff)
        {
            _host.SetMessage(new PluginV2.Messages.RequestChangeConnectionStatus(connectionStatusDiff));
        }

        //public void OnNotifyNormalPluginAdded(NotifyNormalPluginAdded m)
        //{
        //}
        public List<(SiteId, SitePlugin.IOptionsTabPage)> RequestSettingsPanels()
        {
            var answer = _host.RequestMessage(new PluginV2.Messages.ToCore.RequestSettingsPanels());
            if (!(answer is PluginV2.Messages.ToPlugin.AnswerSettingsPanels panels))
            {
        ...
        {
            var reply = _host.RequestMessage(new PluginV2.Messages.RequestConnectionStatus(connId)) as PluginV2.Messages.ReplyConnectionStatus;
            return reply.ConnSt.Name;
        }
        public string GetAppName()
        {
            var reply = _host.RequestMessage(new PluginV2.Messages.RequestAppName()) as PluginV2.Messages.ReplyAppName;
            return reply.AppName;
        }
        public void SetConnectionName(ConnectionId connId, string newConnectionName)
        {
            _host.SetMessage(new PluginV2.Messages.SetConnectionStatus(connId, new ConnectionStatusDiff(connId)
            {
        ...
        {
            _host.SetMessage(new PluginV2.Messages.RequestShowSettingsPanel(pluginId));
        }
