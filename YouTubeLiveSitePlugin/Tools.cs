...
namespace Mcv.YouTubeLiveSitePlugin
{
    static class SapiSidHashGenerator
    {
        ...
        {
            var unixTime = UnixTimeConverter.ToUnixTime(currentDate);
    ...
    static class Tools
    {
        ...
        public static IInput ParseInput(string input)
        {
            ...
            {
                return ChannelUrlTools.CreateChannelUrl(input);
            }
            ...
            {
                return new Vid(input);
            }
            else if (VidResolver.IsWatch(input))
            {
                return new WatchUrl(input);
            }
            else if (VidResolver.IsStudio(input))
            {
                return StudioUrl.CreateStudioUrl(input);
            }
        ...
        public static YtInitialDataOld ExtractYtInitialData(LiveChatHtml liveChatHtml)
        {
            var match = Regex.Match(liveChatHtml.Raw, "window\\[\"ytInitialData\"\\] = ({.+?});");
            if (!match.Success)
            {
                throw new Exception("");
            }
            ...
            {
                throw new SpecChangedException(liveChatHtml.Raw);
            }
            return new YtInitialDataOld(raw);
        }
        public static string ExtractYtCfg(LiveChatHtml liveChatHtml)
        {
            var match = Regex.Match(liveChatHtml.Raw, "ytcfg\\.set\\(({.+?})\\);", RegexOptions.Singleline);
            if (!match.Success)
            {
                throw new ParseException(liveChatHtml.Raw);
            }
            var ytCfg = match.Groups[1].Value;
            dynamic? d = JsonConvert.DeserializeObject(ytCfg);
            var matches = Regex.Matches(liveChatHtml.Raw, "ytcfg\\.set\\(\"([^\"]+)\",\\s*(.+?)\\);?\\r?\n", RegexOptions.Singleline);
