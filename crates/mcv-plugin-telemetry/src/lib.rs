...
pub use mcv_log_schema::capture_stacktrace;
...
impl ErrorContext {
    ...
    pub fn new(message: String, file: &str, line: u32, module_path: &str) -> Self {
        Self {
            ...
            stacktrace: mcv_log_schema::capture_stacktrace(),
            fields: HashMap::new(),
    ...
    // capture_stacktrace() は mcv-log-schema から使用
    ...
    S: tracing::Subscriber,
{
    fn on_event(
        ...
        if level == "error" {
            let frames = mcv_log_schema::capture_stacktrace();
            let stacktrace = serde_json::to_value(&frames).unwrap_or(serde_json::Value::Null);
...
// capture_stacktrace_json() の代わりに、mcv-log-schema::capture_stacktrace() を使用して
// 必要に応じて JSON に変換する
