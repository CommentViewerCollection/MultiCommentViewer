...
impl PluginImplV3Async for OpenrecPlugin {
    async fn on_loaded(&mut self, ctx: PluginContext) {
        let uuid = Uuid::new_v4();
        let plugin_id = PluginId::new(format!("OPENREC_logical_{uuid}"));
        ...
        match mcv_plugin_telemetry::init_tracing(
            ...
            Ok(_) => tracing::trace!(target: "mcv::plugin-openrec", "init_tracing 成功"),
            Err(e) => eprintln!("[plugin-openrec] init_tracing 失敗: {e}"),
        }

        self.initialize(plugin_id);

        let hello = PluginHelloPayload {
            name: "OPENREC".to_string(),
            plugin_id: self.logical_plugin_id.clone(),
            role: vec!["openrec".to_string(), "comment-provider".to_string()],
        ...
        let add_site = AddSitePayload {
            site_id: SiteId::new("OPENREC", "c2d3e4f5-2345-6789-bcde-111111111111"),
            display_name: "OPENREC.tv".to_string(),
...
export_plugin_v3_async!(OpenrecPlugin);
