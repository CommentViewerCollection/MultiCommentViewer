//! YouTube Live state-machine based plugin (experimental).
...
impl PluginImplV3Async for YouTubeLiveStateMachinePlugin {
    async fn on_loaded(&mut self, ctx: PluginContext) {
        ...
        let hello_payload = PluginHelloPayload {
            name: "YouTubeLiveStateMachine".to_string(),
            plugin_id: self.logical_plugin_id,
        ...
        let add_site = AddSitePayload {
            site_id: SiteId::new(
                "YouTubeLiveStateMachine",
                "9f4f0528-9cf4-43d1-b6d4-9e6e19464481",
            ),
            display_name: "YouTube Live (StateMachine)".to_owned(),
